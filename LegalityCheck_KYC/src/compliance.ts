import { GeoService, GeoCheckResult } from '../services/geoService'
...
/**
 * Check if the user has permission to access a specific product
 */
export async function checkProductAccess(
  ...
  try {
    ...
    // 1. Sports information platform: accessible from all regions
    ...
    const sportsPredict = true // Accessible from all regions
    ...
    // If an error occurs, deny access by default
    return {
      sportsPredict: true,
      sportsbook: false,
      etf: false
...
export function complianceMiddleware(productType: 'sportsPredict' | 'sportsbook' | 'etf') {
  return async (req: any, res: any, next: any) => {
    try {
      ...
      // Check access permission for the specific product

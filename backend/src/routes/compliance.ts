...
import { getUserComplianceStatus } from '../middleware/compliance'
 ...
 */
router.post('/identity-verification', authMiddleware, async (req, res) => {
  try {
    const userId = (req as any).user.userId
    const {
      documentType,
 ...
 */
router.get('/identity-verification/status', authMiddleware, async (req, res) => {
  try {
    const userId = (req as any).user.userId
 ...
 */
router.post('/geo-check', authMiddleware, async (req, res) => {
  try {
    const ipAddress = GeoService.getClientIP(req)

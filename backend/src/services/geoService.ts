...
// Regions restricted from using Products 2 and 3
...
export class GeoService {
  ...
  static validateGPSLocation(ipLocation: GeoLocation, gpsLat: number, gpsLon: number): boolean {
    ...
    // If distance exceeds 50km, consider locations as not matching
    return distance <= 50
  }
  ...
  static isRegionRestricted(countryCode: string, regionCode?: string): boolean {
    ...
    // Check Indian state-specific restrictions
  ...
  /**
   * Check if user has permission to use specific product
   */
  static async checkProductAccess(
    ...
    // If region is restricted, identity verification is required
    if (isRestricted) {
      return {
        isAllowed: false,
        region: regionCode,
    ...
    // If GPS information is provided, validate location consistency
    if (gpsLat && gpsLon) {
